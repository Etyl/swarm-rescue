    ...
    while wavefront_value <= np.max(wavefront_map):
        ...
        for i, j in zip(indices[0], indices[1]):
            ...
            for neighbor in neighbors:
                if 0 <= neighbor[0] < wavefront_map.shape[0] and 0 <= neighbor[1] < wavefront_map.shape[1] and \
                        wavefront_map[neighbor] == Zone.EMPTY.value:
...
def find_frontier(wavefront_map, start_row, start_col, visited) -> List[Vector2D]:
    ...
    while stack:
        ...
        if wavefront_map[current_row, current_col] == Zone.INEXPLORED.value and (
        current_row, current_col) not in visited and has_empty_neighbor:
            ...
            neighbors = [(current_row + x, current_col + y) for x in [-1, 0, 1] for y in [-1, 0, 1] if
                         (x != 0 or y != 0)]
